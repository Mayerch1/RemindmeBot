...
#     view = AckView(timeout=timeout)
#     await messageable.respond(embed=eb, view=view)
#     return view.wait()
...
# async def check_user_permission_raw(client, messeagable, guild_id: int, user_roles: list, required_perms:CommunitySettings=None, wait_ack=False, hidden:bool=True):
#     """performs check on if user has sufficient permission to excute the command
#        the user has permission if the server is not a community server
...
#                             hidden=hidden)
#         else:    
#             if isinstance(messeagable, discord.ApplicationContext):
